...
//        "v115@vhKRQJKJJUGJvMJTNJ+DJdAndFndKnFKnFKJXhywHe?wwLeAgH"
    invoke("-t $url -l 8 -n 10 -c default -o output/test.xlsx".split(" "))
//    main("-t $url -l 8 -n 10 -c mipi".split(" "))
}

fun invoke(commands: List<String>) {
    ...
    val runner = MainRunner(minoFactory, colorConverter)

    val data = extractV115(settings.fumen)
    val pages = Tetfu(minoFactory, colorConverter).decode(data)

    val workbook = XSSFWorkbook()
    val maxHeight = settings.height?.takeIf { 0 < it } ?: runner.detectHeight(pages)
    ...
    runner.run(gridSheet, pages, horizontalNum)
...
class MainRunner(private val minoFactory: MinoFactory, private val colorConverter: ColorConverter) {
    fun detectHeight(pages: List<TetfuPage>): Int {
        return pages.maxOf { page ->
            max(
                    page.field.usingHeight,
                    page.colorType?.takeIf { ColorType.isMinoBlock(it) }?.let {
...
private fun properties(colorTheme: String): Properties {
    return Properties().also {
        ...
        it.load(Files.newBufferedReader(Paths.get(path), StandardCharsets.UTF_8))
    }
}